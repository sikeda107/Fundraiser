...
contract("Fundraiser", (accounts) => {
  let fundraiser;
  const name = "Beneficiary Name"; // 受益者名
  const url = "beneficiaryname.org";
  const imageURL = "https://placekitten.com/600/350";
  const description = "Beneficiary description";
  const beneficiary = accounts[1]; // 受益者
  const owner = accounts[0]; //管理人
  ...
  describe("making donations", () => {
    const value = web3.utils.toWei("0.0289"); // 寄付金
    const donor = accounts[2]; // 寄付者アドレス
